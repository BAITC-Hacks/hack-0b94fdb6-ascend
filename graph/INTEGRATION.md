...
Обновление 23.09: по новому запросу владельца поддерживаемая реализация агента находится в `ai_agent/`, операции над графом — в `graph/query.py`. Старые импорты остаются совместимыми. Подключение веб-чата к актуальному backend описано в [ai_agent/README.md](../ai_agent/README.md). Следующие разделы сохраняют историю первоначальной поставки; при расхождении путей используйте это обновление. CONTRACT_AI остаётся нормативным источником аналитических интерфейсов.
