...
- The user's supplied logo and five priority portraits are stored unchanged under `src/assets/`. The portraits appear in the client list, details, priority scale and priority-colored graph. Role/cluster modes use extracted PNG artwork mapped in `src/data/networkArtwork.ts`, shared by the graph, mode-aware list, details and localized legends. The neutral gray export is not shown. Other toolbar and metric icons use the existing icon library.
